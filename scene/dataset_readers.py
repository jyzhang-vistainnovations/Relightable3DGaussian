...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, debug=False):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        image_path_png = os.path.join(
            images_folder, Path(os.path.basename(extr.name)).stem + ".png"
        )
        image_path_jpg = os.path.join(
            images_folder, Path(os.path.basename(extr.name)).stem + ".jpg"
        )
        ...
        if os.path.exists(image_path_png):
            ...
            raise FileNotFoundError(
                f"Could not find image file {image_path_png} or {image_path_jpg}"
            )
        ...
        mask_path = os.path.join(
            ...
            "masks",
            Path(image_path).stem + ".png",  # masks are png
        )
...
def readColmapSceneInfo(path, images, eval, llffhold=8, debug=False):
    ...
    if "DTU" in path and not debug:
        ...
        train_cam_infos = [
            c for idx, c in enumerate(cam_infos) if idx not in test_indexes
        ]
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            if read_mvs:
                depth_path = os.path.join(
                    mvs_dir + "/depths/", os.path.basename(frame["file_path"]) + ".tiff"
                )
                normal_path = os.path.join(
                    mvs_dir + "/normals/", os.path.basename(frame["file_path"]) + ".pfm"
                )
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", debug=False):
    ...
    train_cam_infos = readCamerasFromTransforms(
        path, "transforms_train.json", white_background, extension, debug=debug
    )
    if eval:
        print("Reading Test Transforms")
        test_cam_infos = readCamerasFromTransforms(
            path, "transforms_test.json", white_background, extension, debug=debug
        )
    else:
        test_cam_infos = []
...
def loadCamsFromScene(path, valid_list, white_background, debug):
    ...
    bbox_transform[[0, 1, 2], [0, 1, 2]] = (
        bbox_transform[[0, 1, 2], [0, 1, 2]].max() / 2
    )
    ...
    for i, (index, camera_info) in enumerate(camera_info_list.items()):
        ...
        if camera_info["flg"] == 2:
            ...
            mask_path = os.path.join(
                path + "/pmasks/",
                os.path.basename(image_list[index]).replace(
...
def readNeILFInfo(path, white_background, eval, debug=False):
    ...
    points = pcd.points
    xyz = (
        np.concatenate([points, np.ones_like(points[:, :1])], axis=-1) @ inv_scale_mat.T
    )[:, :3]
    normals = pcd.normals
    colors = pcd.colors
...
def readCamerasFromTransforms2(
    path,
    transformsfile,
    white_background,
    extension=".png",
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            mask_item = (
                frame["file_path"]
                .replace("test", "test_mask")
                .replace("train", "train_mask")
            )
            ...
            image = cv2.resize(
                image, (benchmark_size, benchmark_size), interpolation=cv2.INTER_AREA
            )
            mask = cv2.resize(
                mask, (benchmark_size, benchmark_size), interpolation=cv2.INTER_AREA
            )
...
def readStanfordORBInfo(
    ...
    if eval:
        print("Reading Test Transforms")
        test_cam_infos = readCamerasFromTransforms2(
            path,
...
def readCamerasFromTransforms3(
    path, transformsfile, white_background, extension=".png", debug=False
):
    cam_infos = []
...
def readSynthetic4RelightInfo(path, white_background, eval, debug=False):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms3(
        path, "transforms_train.json", white_background, "_rgb.exr", debug=debug
    )
    if eval:
        print("Reading Test Transforms")
        test_cam_infos = readCamerasFromTransforms3(
            path, "transforms_test.json", white_background, "_rgba.png", debug=debug
        )
    else:
        test_cam_infos = []
