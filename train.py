...
def training(
    ...
    # Save depth maps for training views
    save_depth(scene, gaussians, render_fn, pipe, background, opt, pbr_kwargs)
...
def save_depth(scene, gaussians, render_fn, pipe, background, opt, pbr_kwargs):
    """Save depth maps for all training views."""
    train_cameras = scene.getTrainCameras()
    depth_dir = os.path.join(args.model_path, "depth")
    os.makedirs(depth_dir, exist_ok=True)

    print("Saving depth maps for training views...")
    with torch.no_grad():
        for idx, viewpoint in enumerate(tqdm(train_cameras, desc="Saving depth maps")):
            results = render_fn(
                viewpoint,
                gaussians,
                pipe,
            ...
            save_image(
                depth.repeat(3, 1, 1),
                os.path.join(depth_dir, f"{viewpoint.image_name}.png"),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--type", choices=["render", "normal", "neilf"], default="render"
    )
